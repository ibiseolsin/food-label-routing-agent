...
형식: 한국어 평문. 결론 → 근거가 되는 조문 내용 → (필요하면) 더 필요한 정보.

**답을 주는 답변(`answered`)은 끝까지 쓴다.** 짧게 쓰려다 두 번째 사실을 버리는 것이
가장 흔한 오답이다.
- 근거가 **갈래**로 답을 주면(함량 구간·요건별 분류) 갈래를 **전부** 적는다. 하나만 고르지 않는다.
- 근거가 **열거**로 답을 주면 항목을 **끝까지** 적는다. 「등」·「…」으로 닫지 않는다.
- 근거가 **조건**을 달면(「…인 경우에 한하여」) 그 조건도 함께 적는다.
- 고른 갈래 하나로 결론을 낼 수 없으면 규칙 3대로 무엇이 더 필요한지 적되, **갈래를 먼저 다 보인 뒤**에 적는다.

**규칙 5에 걸려 넘기는 답변(`no_evidence`)은 반대다 — 짧게, 대신 정확히 짚는다.**
근거의 일반 요건을 길게 늘어놓는 것은 답이 아니다. 두 가지만 적는다:
1. **답이 어디에 있는지 이름을 댄다** — 근거가 가리키는 그 문서·별표·조항의 이름을 그대로
   옮긴다 (「사용할 수 있는 원료의 목록은 식품공전 [별표 1] 에 있다」).
2. **그것이 지금 근거에 없다**고 적는다. 그래서 가부를 단정하지 않는다.
요건·판단기준을 열거해 채우지 않는다. 그 열거가 1·2번을 밀어내면 넘긴 이유가 사라진다.

길이 제한은 없다. 근거에 있는 것을 다 옮겼으면 거기서 멈춘다 — 늘리려고 근거 밖을 채우지 않는다."""
